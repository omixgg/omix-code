...
const { ChatController, OmixCodeChatViewProvider, OmixCodeChatPanelManager } = require('./chat/chatProvider');
const { SessionManager } = require('./chat/sessionManager');
const { DiffContentProvider, SCHEME: DIFF_SCHEME } = require('./chat/diffController');

const OmixCode_REPO_URL = 'https://github.com/omixgg/OmixCode';
const OmixCode_SETUP_URL = 'https://github.com/omixgg/OmixCode/blob/main/README.md#quick-start';
const PROFILE_FILE_NAME = '.OmixCode-profile.json';
...
async function collectControlCenterState() {
  const configured = vscode.workspace.getConfiguration('OmixCode');
  const launchCommand = configured.get('launchCommand', 'OmixCode');
  const terminalName = configured.get('terminalName', 'OmixCode');
...
async function launchOmixCode(options = {}) {
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `OmixCode command not found: ${executable}. Install it with: npm install -g @omixgg/OmixCode`,
      'Open Setup Guide',
      'Open Repository',
    );

    if (action === 'Open Setup Guide') {
      await vscode.env.openExternal(vscode.Uri.parse(OmixCode_SETUP_URL));
    } else if (action === 'Open Repository') {
      await vscode.env.openExternal(vscode.Uri.parse(OmixCode_REPO_URL));
    }
...
function getWorkspaceRootActionDetail(status, fallbackDetail) {
  ...
  if (status.launchActionsShareTargetReason === 'relative-launch-command') {
    return `Same workspace-root target as Launch OmixCode because the relative command resolves from the workspace root · ${status.workspaceRootCwdLabel}`;
  }
      ...
      <header class="hero">
        <div class="hero-top">
          <div class="brand">
            ...
            <div class="wordmark" aria-label="OmixCode wordmark">Open<span class="wordmark-accent">Claude</span></div>
            <div class="headline">
        ...
        <section class="support-card" aria-labelledby="quick-links-title">
          ...
          <div class="support-stack">
            ...
            <button class="support-link" id="repo" type="button">
              ...
              <span class="summary-detail">Browse the upstream OmixCode project.</span>
            </button>
            <button class="support-link" id="commands" type="button">
              <span class="support-link-label">Open Command Palette</span>
              <span class="summary-detail">Access VS Code and OmixCode commands quickly.</span>
            </button>
          </div>
        </section>
      </section>
...
class OmixCodeControlCenterProvider {
  constructor() {
  ...
  async resolveWebviewView(webviewView) {
    ...
    webviewView.webview.onDidReceiveMessage(async message => {
      switch (message?.type) {
        case 'launch':
          await launchOmixCode();
          break;
        case 'launchRoot':
          await launchOmixCode({ requireWorkspace: true });
          break;
        case 'openProfile':
...
function activate(context) {
  ...
  const provider = new OmixCodeControlCenterProvider();
  ...
  const chatViewProvider = new OmixCodeChatViewProvider(chatController);
  const chatPanelManager = new OmixCodeChatPanelManager(chatController);
  ...
  statusBarItem.text = '$(comment-discussion) OmixCode';
  statusBarItem.tooltip = 'Open OmixCode Chat';
  statusBarItem.command = 'OmixCode.openChat';
  statusBarItem.show();

  chatController.onDidChangeState((state) => {
    switch (state) {
      case 'streaming':
        statusBarItem.text = '$(sync~spin) OmixCode';
        statusBarItem.tooltip = 'OmixCode is generating...';
        break;
      case 'connected':
        statusBarItem.text = '$(comment-discussion) OmixCode';
        statusBarItem.tooltip = 'OmixCode connected';
        break;
      default:
        statusBarItem.text = '$(comment-discussion) OmixCode';
        statusBarItem.tooltip = 'Open OmixCode Chat';
  ...
  // ── Existing commands ──
  const startCommand = vscode.commands.registerCommand('OmixCode.start', async () => {
    await launchOmixCode();
  });

  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'OmixCode.startInWorkspaceRoot',
    async () => {
      await launchOmixCode({ requireWorkspace: true });
    },
  );

  const openDocsCommand = vscode.commands.registerCommand('OmixCode.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(OmixCode_REPO_URL));
  });

  const openSetupDocsCommand = vscode.commands.registerCommand(
    'OmixCode.openSetupDocs',
    async () => {
      await vscode.env.openExternal(vscode.Uri.parse(OmixCode_SETUP_URL));
    },
  );
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'OmixCode.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('OmixCode.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.OmixCode');
  });

  // ── New chat commands ──
  const newChatCommand = vscode.commands.registerCommand('OmixCode.newChat', () => {
    chatController.stopSession();
    chatController.broadcast({ type: 'session_cleared' });
  });

  const openChatCommand = vscode.commands.registerCommand('OmixCode.openChat', () => {
    chatPanelManager.openPanel();
  });

  const resumeSessionCommand = vscode.commands.registerCommand('OmixCode.resumeSession', async () => {
  ...
  const abortChatCommand = vscode.commands.registerCommand('OmixCode.abortChat', () => {
    chatController.abort();
  });

  // ── Register providers ──
  const controlCenterProviderReg = vscode.window.registerWebviewViewProvider(
    'OmixCode.controlCenter',
    provider,
  );
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('OmixCode')) {
        refreshProvider();
      }
    }),
...
module.exports = {
  activate,
  deactivate,
  OmixCodeControlCenterProvider,
  renderControlCenterHtml,
  resolveLaunchTargets,
  ChatController,
  OmixCodeChatViewProvider,
  OmixCodeChatPanelManager,
};
