# OmixCode Local Agent Playbook

This playbook is a practical guide to run OmixCode with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, OmixCode starts directly.
...
Or let OmixCode recommend the best local model for your goal:
...
```powershell
Get-Content .\.OmixCode-profile.json
```
...
- Keep any legacy workspace `.OmixCode-profile.json` local; new provider saves use the user config directory.
