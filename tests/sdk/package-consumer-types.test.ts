...
function setupConsumerProject(name: string): string {
  ...
  // Simulate node_modules/@omixgg/OmixCode structure
  const pkgDir = join(tmpDir, 'node_modules', '@omixgg', 'OmixCode')
...
describe('package consumer types', () => {
  ...
  test('SDKMessage/SDKUserMessage/SDKResultMessage are re-exported correctly', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import type { SDKMessage, SDKUserMessage, SDKResultMessage } from '@omixgg/OmixCode/sdk'`,
        ``,
  ...
  test('SDKRateLimitError has resetsAt and rateLimitType as class properties', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import { SDKRateLimitError } from '@omixgg/OmixCode/sdk'`,
        ``,
