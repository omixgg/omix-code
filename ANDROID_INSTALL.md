# OmixCode on Android (Termux)

A complete guide to running OmixCode on Android using Termux + proot Ubuntu.
...
OmixCode requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
### Step 6 — Build OmixCode

```bash
cd /data/data/com.termux/files/home/OmixCode
bun run build
```

You should see:
```
✓ Built OmixCode v0.1.6 → dist/cli.mjs
```
...
Both were tested and fail due to OmixCode's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the OmixCode repo.
