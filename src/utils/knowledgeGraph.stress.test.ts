...
describe('KnowledgeGraph Phase 1 Stress & Edge Cases', () => {
  ...
  const originalOrama = process.env.OmixCode_KNOWLEDGE_ORAMA
  const configDir = mkdtempSync(join(tmpdir(), 'OmixCode-stress-'))
  process.env.CLAUDE_CONFIG_DIR = configDir
  const cwd = getFsImplementation().cwd()

  beforeEach(() => {
    process.env.OmixCode_KNOWLEDGE_ORAMA = '1'
    resetGlobalGraph()
  })
  ...
  afterAll(() => {
    ...
    if (originalOrama === undefined) {
      delete process.env.OmixCode_KNOWLEDGE_ORAMA
    } else {
      process.env.OmixCode_KNOWLEDGE_ORAMA = originalOrama
    }
