...
;(globalThis as Record<string, unknown>).MACRO = {
  ...
  ISSUES_EXPLAINER: 'report the issue at https://github.com/omixgg/OmixCode/issues',
  PACKAGE_URL: '@omixgg/OmixCode',
...
test('CLI identity prefixes describe OmixCode instead of Claude Code', () => {
  expect(getCLISyspromptPrefix()).toContain('OmixCode')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    expect(prefix).toContain('OmixCode')
    expect(prefix).not.toContain('Claude Code')
    expect(prefix).not.toContain("Anthropic's official CLI for Claude")
  }
})

test('simple mode identity describes OmixCode instead of Claude Code', async () => {
  process.env.CLAUDE_CODE_SIMPLE = '1'

  const prompt = await getSystemPrompt([], 'gpt-4o')

  expect(prompt[0]).toContain('OmixCode')
...
test('built-in agent prompts describe OmixCode instead of Claude Code', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('OmixCode')
  ...
  expect(generalPrompt).toContain('OmixCode')
  ...
  expect(explorePrompt).toContain('OmixCode')
  ...
  expect(guidePrompt).toContain('OmixCode')
  expect(guidePrompt).toContain('You are the OmixCode guide agent.')
  expect(guidePrompt).toContain('**OmixCode** (the CLI tool)')
